#include<stdio.h>
#include<iostream>//unsigned long type support
using namespace std;
#include<stdlib.h>
#include<string.h>
#include<conio.h>
...
void FileSplitGroup(char * file, unsigned long bytesize, int delsource);
void FileGroupCombine(char * mainfile, int delsource);
int main(int argc, char * argv[])
{
	system("title 文件强制拆分工具 by Ice2Faith");
	system("color f1");
	unsigned long  maxsizefile = 1024 * 1024;
	int delsource = 1;
	char file[2048] = { 0 };
	while (1)
	{
		...
		case 1:
		{
				  ...
				  while (maxsizefile < 1024 || maxsizefile >= 4294967295)
				  {
					  cout << "请认真一点重新输入，范围不在1KB-2047MB之间\n>/ ";
					  cin >> maxsizefile;
				  }
...
void FileSplitGroup(char * file, unsigned long  bytesize, int delsource)
{
	...
	unsigned long sumcount = ftell(sfile);
	...
	char node[20] = { 0 };
	for (unsigned long i = 1; i < sumcount; i++)
	{
	...
	fwrite(&sumcount, sizeof(unsigned long), 1, gfile);
...
void FileGroupCombine(char * mainfile, int delsource)
{
	...
	fread(&sumcount, sizeof(unsigned long), 1, gfile);
	for (int i = 0; i < sumcount; i++)
	{
		int lenname = strlen(newfile[i]);
		fread(&lenname, sizeof(int), 1, gfile);
