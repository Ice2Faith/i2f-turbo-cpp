...
void TraceMouseStar::DrawBackground(HDC hdc)
{
	...
	//bitblt的效率实在是太低了，在这样的高刷新率环境下是在不行
	//因此，隔几次拷贝一次，实际效果也不会有明显感觉
	//实现维持刷新率
	...
	for (int i = 0; i < mMaxCount; i++)
	{
		if (mNode[i].live)
		{
			...
			size *= mNode[i].curTime*1.0 / mNode[i].maxTimes;//实现大小缩放
	...
	if (isDown==false)
	{
		...
		int diff = rand() % 7500 + 500;//时间随机到500-8000ms,也就是0.5-8秒之内随机
