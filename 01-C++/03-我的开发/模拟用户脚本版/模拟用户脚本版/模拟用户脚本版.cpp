...
{
	cleardevice();
	GPrint(0, 0, "模拟用户脚本版正在运行...");
}
void FormatWindow()
{
	//窗口最大化
	hwnd = initgraph(screenx, screeny, SHOWCONSOLE);
	//窗口透明
...
void ReadClickEvent(char * name)
{
	...
	while (1)
	{
		GPrint(0, 40, "Control+Alt+Space切换状态...");
		if (IsKeyDown(VK_CONTROL) && IsKeyDown(VK_MENU) && IsKeyDown(VK_SPACE))
			pass *= -1;
		if (pass == -1)
		{
			//不穿透点击
		...
		system("cls");
		FlushScreen();
		sprintf_s(temp, "Point:%d,%d\0", point.x, point.y);
		GPrint(0, 80, temp);
